...
export default {
  ...
  // ✅ Correct key name for the backend serializer
  const items = this.cartItems.map((i) => ({
    item: i.id, // not "product"
    quantity: i.qty,
  }));
  ...
  try {
    ...
    if (this.paymentProofFile) {
      ...
      items.forEach((it, i) => {
      fd.append(`items[${i}][item]`, it.item);
      fd.append(`items[${i}][quantity]`, it.quantity);
      });      
      fd.append("payment_proof", this.paymentProofFile);
      res = await axios.post("http://127.0.0.1:8000/api/orders/", fd, {
    ...
    if (res.status === 201 || res.status === 200) {
      ...
      setTimeout(() => this.$router.push({ name: "Orders" }), 1400);
    } else {
