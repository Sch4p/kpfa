  ...
  <div class="home-page">
    ...
    >
      ...
      <div class="hero-content">
        ...
        <p class="hero-subtitle">{{ $t('banner.building_champions') }}</p>
        <div class="hero-buttons mt-4">
          <router-link to="/join-us" class="btn btn-lg btn-primary me-3" style="margin-bottom: 5px;">{{ $t('buttons.join') }}</router-link>
          <router-link to="/teams" class="btn btn-lg btn-outline-light me-3" style="margin-bottom: 5px;">{{ $t('buttons.view_teams') }}</router-link>
    ...
    <div class="container my-5">
      ...
      <section class="about-snippet text-center mb-5">
        ...
        <p class="lead text-muted">
          {{ $t('home_subheaders.about_us_cont')}}
        </p>
        ...
        <div class="row g-4">
          <div class="col-md-4">
            <div class="card team-card shadow-sm border-0 h-100">
              ...
              <div class="card-body text-center">
                ...
                <p class="card-text text-muted">{{ $t('our_team.men_squad') }}</p>
          ...
          <div class="col-md-4">
            <div class="card team-card shadow-sm border-0 h-100">
              ...
              <div class="card-body text-center">
                <h5 class="card-title fw-semibold">Kutty Philips FC</h5>
                <p class="card-text text-muted">{{ $t('our_team.women_squad') }}</p>
        ...
        <!-- Junior Team Card -->
          <div class="col-md-4">
            <div class="card team-card shadow-sm border-0 h-100">
              ...
              <div class="card-body text-center">
                <h5 class="card-title fw-semibold">Kutty Philips FC</h5>
                <p class="card-text text-muted">{{ $t('our_team.junior_squad') }}</p>
...
<section v-if="newsLoaded && randomVideo" class="latest-news mb-5">
  <h2 class="section-title text-center mb-4 text-success" >
    {{ $t('home_subheaders.news_videos') }}
  </h2>
...
<div class="row align-items-start">
  <div class="row g-4">
   ...
   <!-- <div class="col-md-4 d-flex">-->
      <div class="card shadow-sm w-100" style="height: 350px;">
        <img v-if="news[0]?.image" :src="news[0].image" class="card-img-top" alt="News image" style="height: 180px; object-fit: cover;" />
        <div class="card-body d-flex flex-column">
          <h5 class="card-title">{{ news[0]?.title || 'Untitled' }}</h5>
          <p class="card-text text-muted" style="flex-grow: 1; overflow: hidden;">
            {{ news[0]?.excerpt || news[0]?.summary || (news[0]?.content?.slice(0, 120) + '...') || '' }}
          </p>
          <router-link :to="`/news/${news[0]?.id || ''}`" class="btn btn-sm btn-success mt-auto">
            {{ $t('news.read_more') }}
          </router-link>
        </div>
      </div>
    ...
    <!-- Video -->
    <div class="col-lg-8 col-md-12 mb-4 d-flex justify-content-center">
    <!--<div class="col-md-8 d-flex justify-content-center">-->
      <div style="width: 100%; max-width: 100%; height: 350px; overflow: hidden;">
        <YouTubeFloating v-if="randomVideo" :video-id="randomVideo.id" style="width: 100%; height: 100%;" />
      </div>
    </div>
  </div>
  <!--</div>-->
    ...
    <h2 class="text-center mb-4 fw-bold text-warning">{{ $t('home_subheaders.kpfa_eshop') }}</h2>
            ...
            >
              ...
              <div class="card-body text-center">
                ...
                <div class="d-flex justify-content-center gap-2">
                  <button class="btn btn-sm btn-outline-success" @click.stop="addToCart(item)">{{ $t('buttons.add_to_cart') }}</button>
                  <router-link :to="`/eshop/${item && (item.slug || item.id) || ''}`" class="btn btn-sm btn-success">{{ $t('buttons.view') }}</router-link>
                </div>
              </div>
            </div>
...
<!-- Modern Slide-In Mini Cart -->
<transition name="slide-cart">
  <div 
    v-if="showCart"
    class="floating-cart shadow-lg border bg-white rounded p-3"
  >
    <div class="d-flex justify-content-between align-items-center mb-3">
      <h5 class="m-0">🛒 My Cart ({{ cartCount }})</h5>
      <button class="btn btn-sm btn-outline-danger" @click="toggleCart">
        ✕
      </button>
    </div>

    <div v-if="cart.length === 0" class="text-muted text-center">
      Your cart is empty
    </div>
...
<div class="text-center my-4">
    ...
    style="
      ...
      background-color: #FF8C00;  /* orange ribbon */
      color: white;
      padding: 0.25rem 1rem;      /* controls height and width */
      font-size: 1rem;
      line-height: 1.5;
      border-radius: 25px;
      font-weight:normal;
  ...
  <div v-if="upcomingMatch" class="card shadow-sm p-4 border-0 mx-auto" style="max-width: 600px; background-color: #fff;">
    <div class="row align-items-center text-center">
      ...
      <div class="col-5 d-flex flex-column align-items-center">
        <img
          :src="teamLogo(upcomingMatch.home_team)"
          alt="Home Team Logo"
      ...
      <div class="col-5 d-flex flex-column align-items-center">
        <img
          :src="teamLogo(upcomingMatch.away_team)"
          alt="Away Team Logo"
          class="img-fluid mb-2"
          style="max-width: 70px;"
        />
...
import YouTubeFloating from "@/components/YouTubeFloating.vue";
import { emitter } from "@/utils/emitter"; // same emitter used in News.vue
...
setup() {
  ...
  const loadCart = () => {
    const saved = JSON.parse(localStorage.getItem("kpfa_cart")) || [];
    cart.value = saved;
    cartCount.value = saved.reduce((sum, item) => sum + item.qty, 0);
  };
  ...
  const addToCart = (item) => {
    ...
    const saved = JSON.parse(localStorage.getItem("kpfa_cart")) || [];
    const existing = saved.find((i) => i.id === item.id);
    if (existing) {
      existing.qty += 1;
    } else {
      saved.push({
    ...
    localStorage.setItem("kpfa_cart", JSON.stringify(saved));
    cart.value = saved;
    cartCount.value = saved.reduce((sum, i) => sum + i.qty, 0);

   // alert(`${item.name} added to cart!`);
  };

  return { showCart, cart, cartCount, toggleCart, goToCheckout, addToCart };
},
  ...
  { 
    id: "yEcxh0gEtjE",
    title: "Full Training Session for Soccer",
    description: "Comprehensive football training session covering ball control, passing, and shooting drills.",
  },
  { 
    id: "zQdmPW018LQ",
    title: "Ball Mastery Training Session",
    description: "Focused drills to master ball handling, control, and agility for academy players.",
  },
  { 
    id: "art5mDfe9tE",
    title: "Full Group Football Training",
    description: "Team-oriented drills and exercises for group coordination and tactical awareness.",
  }
]
    };
  },
  ...
  methods: {
    ...
    async translateHomeDynamicFields(targetLang) {
    if (this.translateQueueLock) return;
    this.translateQueueLock = true;
    try {
      // Upcoming match location
      if (this.upcomingMatch?.location) {
        this.upcomingMatch.location = await this.translateText(
          this.upcomingMatch.location,
          targetLang
        );
      }
      ...
      if (this.news?.length > 0) {
        const newsItem = this.news[0];

        if (newsItem.title) {
          newsItem.title = await this.translateText(newsItem.title, targetLang);
        }

        const excerptText =
          newsItem.excerpt ||
          newsItem.summary ||
          (newsItem.content ? newsItem.content.slice(0, 120) + "..." : "");

        newsItem.excerpt = await this.translateText(excerptText, targetLang);
      }
    } catch (err) {
      console.error("Home dynamic translation error:", err);
    } finally {
  ...
  async mounted() {
    ...
    this.getRandomVideo();
    // Subscribe to global navbar language changes
  emitter.on("locale-changed", this.translateHomeDynamicFields);
...
.floating-cart {
  ...
  max-height: 400px; /* scroll if too many items */
  background: #fff;
...
.upcoming-banner {
  ...
  line-height: 1.5;
  text-align: center;
  border-radius: 20px; /* optional: match button rounded corners */
}
...
.latest-news {
  padding-top: 2rem;  /* space above the heading */
  padding-bottom: 1rem; /* space below the cards */
}
...
@media (max-width: 992px) {
  .latest-news .row.align-items-start > .col-md-12 {
    margin-bottom: 1rem; /* vertical spacing between stacked cards */
  }
}
...
@media (max-width: 576px) {
  .latest-news h2.section-title {
    margin-bottom: 1rem; /* more spacing under heading */
  }
}
