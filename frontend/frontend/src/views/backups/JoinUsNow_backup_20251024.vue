...
<div class="col-md-6">
  ...
  <div v-if="errors.state_of_origin" class="text-warning small">{{ $t('form.errors.state_of_origin') }}</div>
          ...
          <div class="mb-3">
            ...
            <select v-model="form.team" class="form-select form-select-sm" use :placeholder="$t('form.holder.select_team')">
...
export default {
  ...
  data() {
    return {
      ...
      countries: ["Nigeria", "Ghana", "Togo", "Benin", "Ivory Coast"],
    countryStates: {
      Nigeria: [
  ...
  // Listen for global translation change
      this.$root.$on("language-changed", this.translateJoinUsNow);
      },
