<template>
  <div class="home-page">
    ...
    <section
      ...
      :style="heroStyle" content="fit"
      aria-label="Hero"
    >
      <div class="overlay"></div>
      <div class="hero-content">
        <h1 class="hero-title">Kutty Philips Football Academy</h1>
        <p class="hero-subtitle">Building Champions of Tomorrow</p>
        <div class="hero-buttons mt-4">
          <router-link to="/join-us" class="btn btn-lg btn-primary me-3">Join Us Now</router-link>
          <router-link to="/teams" class="btn btn-lg btn-outline-light me-3">View Teams</router-link>
          <router-link to="/eshop" class="btn btn-lg btn-success">Visit eShop</router-link>
        </div>
      </div>
    </section>
    ...
    <div class="container my-5">
      ...
      <section class="about-snippet text-center mb-5">
        ...
        <router-link to="/about" class="btn btn-success mt-3">Read More</router-link>
      </section>

      <!-- Featured Teams -->
      <section v-if="teamsLoaded" class="mb-5">
        <h2 class="text-center section-title mb-4 fw-bold">Our Teams</h2>
        <div class="row g-4">
          <div
            class="col-md-4"
            v-for="(team, idx) in (teams || []).slice(0, 3)"
            :key="team && team.id ? team.id : `team-${idx}`"
          >
            <div class="card team-card shadow-sm border-0 h-100">
              <div class="card-img-wrapper">
                <img
                  :src="(team && team.image) || '/images/team-placeholder.jpg'"
                  class="card-img-top"
                  :alt="(team && team.name) || 'Team image'"
                />
              </div>
              <div class="card-body text-center">
                <h5 class="card-title fw-semibold">{{ (team && team.name) || 'Team' }}</h5>
                <p class="card-text text-muted">{{ (team && team.description) || 'Team description coming soon.' }}</p>
                <router-link :to="`/teams/${team && (team.slug || team.id) || ''}`" class="btn btn-outline-success">
                  View Team
                </router-link>
              </div>
            </div>
          ...
          >
            <div class="card h-100 shadow-sm">
              ...
              <div class="card-body d-flex flex-column">
                ...
                <div class="mt-auto">
                  <router-link :to="`/news/${article && article.id || ''}`" class="btn btn-sm btn-primary">Read More</router-link>
                </div>
              </div>
            </div>
          </div>
      ...
      <section v-if="displayShop.length" class="mb-5 position-relative">
        ...
        <div class="text-center mt-3">
          <router-link to="/eshop" class="btn btn-success btn-lg me-2">View All Products</router-link>
        </div>
      </section>

      <!-- Upcoming Match (only nearest) -->
      <section class="upcoming-match mb-5">
        <h2 class="section-title text-center">Upcoming Match</h2>
        <div v-if="nextMatch" class="d-flex justify-content-center">
          <FixtureBox
        ...
        <router-link to="/matches" class="btn btn-outline-secondary mt-3 d-block mx-auto">View All Matches</router-link>
      </section>
    ...
    <section class="cta-footer text-white text-center py-5 rounded-4" style="background:#006400;">
      <h3 class="mb-3">Ready to join the next generation of champions?</h3>
      <router-link to="/join-us" class="btn btn-light btn-lg mt-3">Join Us Now</router-link>
    </section>
  ...
  methods: {
    ...
    async _fetchAll() {
      try {
        ...
        // normalize teams
        this.teams = Array.isArray(teamsRes.data) ? teamsRes.data : teamsRes.data.results ?? [];
        this.teamsLoaded = true;

        // normalize news
        this.news = Array.isArray(newsRes.data) ? newsRes.data : newsRes.data.results ?? [];
        ...
        // normalize matches and compute nearest upcoming
        const matchesArr = Array.isArray(matchesRes.data) ? matchesRes.data : matchesRes.data.results ?? [];
        this.matches = matchesArr;
        this.matchesLoaded = true;

        // compute nearest upcoming (defensive)
        if (Array.isArray(this.matches) && this.matches.length > 0) {
          const now = new Date();
          const upcoming = this.matches.filter(m => m && m.date && new Date(m.date) > now);
          upcoming.sort((a, b) => new Date(a.date) - new Date(b.date));
          this.nextMatch = upcoming.length ? upcoming[0] : null;
        } else {
          this.nextMatch = null;
        }
      } catch (err) {
        console.error("Home page fetch error:", err);
        // leave arrays empty; components will show fallback text
      }
    },
